...
- **Amortization schedule**: manual add/remove/clear, or auto-generate by splitting nominal into N equal parts with dynamic amount preview; multi-edit support
- **Coupon schedule**: manual add/remove/clear, or auto-generate by period (30/90/180 day presets); generation respects amortizations (reducing nominal); rate (%) and amount two-way sync; multi-edit support
- **Deal entry**: clean price, accrued interest (ACI / НКД), purchase date, annual inflation rate
...
### 4. Deal
Enter the **clean price** you paid, **accrued interest** (НКД), **purchase date**, and the expected **annual inflation rate** (in percent).

### 5. Calculate
Press **Calculate** to compute all results, the schedule table, and the chart.

### 6. Export
Use **Export > As TXT** from the menu to save a formatted report.

### 7. Language
Switch between English and Russian via the **Language** menu. The choice is saved and restored on next launch.
